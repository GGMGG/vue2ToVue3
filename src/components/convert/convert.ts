 ...
 * author: wd3322
 * address: https://github.com/wd3322/vue2-to-composition-api
 * desc：copy from wd3322 and do something change
 */
...
function Vue2ToCompositionApi(
  ...
  let result = { isSuccess: false, msg: "" };
  if (getPrototype(entryScriptContent) !== "string") {
    result.isSuccess = false;
    result.msg = "需要转换的内容类型不为string";
    return;
  }
  ...
  try {
    ...
    const beautifyScriptContent: string = entryScriptContent;
    const modelScriptContent: string | undefined = (function () {
      ...
      return beautifyScriptContent.match(braceRegExp)?.[0].replace(componentsRegExp, "").replace(mixinsRegExp, "");
    })();
    ...
    for (const prop in vmBody) {
      if (
        ["beforeCreate", "created", "beforeMount", "mounted", "beforeUpdate", "updated", "beforeDestroy", "destroyed", "activated", "deactivated", "errorCaptured"].includes(
          prop
        ) &&
      ...
      computed(): void {
        if (vmKeys.computed.length > 0) {
          ...
          for (const prop in vmContent.computed) {
            ...
            if (computedContent && ["object", "function", "asyncfunction"].includes(getPrototype(computedContent))) {
      ...
      watch(): void {
        if (vmKeys.watch.length > 0) {
          ...
          for (const prop in vmContent.watch) {
            ...
            if (getPrototype(watchContent).indexOf("function") !== -1) {
            ...
            } else if (watchContent && getPrototype(watchContent) === "object" && getPrototype(watchContent.handler).indexOf("function") !== -1) {
              ...
              if (watchName && watchFunctionStr && watchOptionsStr) {
                watchValues.push(
                  watchOptionsStr !== "{}" ? `watch(() => ${watchName}, ${watchFunctionStr}, ${watchOptionsStr})` : `watch(() => ${watchName}, ${watchFunctionStr})`
                );
              }
            }
          }
          ...
          for (const prop in vmContent.hooks) {
            ...
            if (getPrototype(hookContent).indexOf("function") !== -1) {
              if (["beforeCreate", "created"].includes(hookContent.name)) {
                ...
                ["beforeMount", "mounted", "beforeUpdate", "updated", "beforeDestroy", "destroyed", "activated", "deactivated", "errorCaptured"].includes(hookContent.name)
              ) {
                ...
                const hookFunctionStr: string = utilMethods.getContentStr(hookContent, null, {
                  function: (params: any) => {
                    const { type, value, arg, body } = params;
                    if (type === "custom") {
                      return value.constructor.name === "AsyncFunction" ? `${hookName} (async ${arg} => ${body})` : `${hookName} (${arg} => ${body})`;
                    }
                  },
                });
                if (hookName && hookFunctionStr) {
            ...
            if (getPrototype(methodContent).indexOf("function") !== -1) {
              ...
              const methodFunctionStr: string = utilMethods.getContentStr(methodContent, null, {
                function: (params: any) => {
                  const { type, value, arg, body } = params;
                  if (type === "custom") {
                    return value.constructor.name === "AsyncFunction" ? `async function ${methodName} ${arg} ${body}` : `function ${methodName} ${arg} ${body}`;
                  }
                },
              });
            ...
            if (getPrototype(filterContent).indexOf("function") !== -1) {
              ...
              const filterFunctionStr: string = utilMethods.getContentStr(filterContent, null, {
                function: (params: any) => {
                  const { type, value, arg, body } = params;
                  if (type === "custom") {
                    return value.constructor.name === "AsyncFunction" ? `async function ${filterName} ${arg} ${body}` : `function ${filterName} ${arg} ${body}`;
                  }
                },
              });
    ...
    const utilMethods: UtilsMethods = {
      getContentStr(
        ...
        // function prototype
          ...
          // native code
          if (["String", "Number", "Boolean", "Array", "Object", "Date", "Function", "Symbol"].includes(value.name) && content.match(braceRegExp)?.[0] === "{ [native code] }") {
            result = `${value.name}`;
          ...
          // custom code
            ...
            result = value.constructor.name === "AsyncFunction" ? `async ${arg} => ${body}` : `${arg} => ${body}`;
            if (resultCallbackContent.function) {
      ...
      replaceValue(value: string, dataKeyToUseData: boolean = false): string {
        ...
        result = value
          .replace(thisKeyRegExp, function (str: string, separator: string, key: string, index: number, content: string): string {
            // props key
            if (vmKeys.props.includes(key)) {
              return "props." + key;
            }
            ...
            // emit key
              ...
              return name ? key.substring(1) : `/* Warn: Cannot find emit name */ $vm.$emit`;
            }
            // refs key
            else if (key === "$refs") {
              ...
              return name ? "$REFS_KEY" : `/* Warn: Cannot find refs name */ $vm.$refs`;
            }
            // other key
            else if (
              [
          ...
          .replace(refKeyRegExp, function (str: string, separator: string, name: string): string {
            // reset refs key
            return name + ".value";
          });
        return result;
    ...
    result.msg = outputScriptContent;
    return result;
  } catch (err: any) {
    result.isSuccess = true;
    result.msg = err;
